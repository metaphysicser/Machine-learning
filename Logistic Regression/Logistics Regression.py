# coding: utf-8
"""
@date:created on 29th July 29th 2020
@author:张平路
@function:realize the Logistic Regression Algriothm
...
class LinearLogsiticRegression(object):
    ...
    def fit(self, X, y, alpha=0.01, accuracy=0.00001):
        # 插入第一列为1，构成xb矩阵
        self.thetas = np.full(X.shape[1] + 1, 0.5)#all num is 0.5
        self.m = X.shape[0]
        a = np.full((self.m, 1), 1)
        Xb = np.column_stack((a, X))#intrgrate a and X
        ...
        while True:
            ...
            for j in range(dimension):
                self.thetas[j] = self.thetas[j] - alpha * np.sum(c * Xb[:, j])#update theta
