...
"""
@date:created on July 31th on 16;42 2020
@author：张平路
@function:realize the Naivebayes
...
def spamTest():
    ...
    for i in range(1, 26):  # 遍历25个txt文件
        wordList = textParse(
            open('垃圾邮箱/spam/%d.txt' % i, 'r').read())  # 读取每个垃圾邮件，并字符串转换成字符串列表

        docList.append(wordList)
        fullText.append(wordList)
        classList.append(1)  # 标记垃圾邮件，1表示垃圾文件
        wordList = textParse(
            open('垃圾邮箱/ham/%d.txt' % i, 'r').read())  # 读取每个非垃圾邮件，并字符串转换成字符串列表
        docList.append(wordList)
        fullText.append(wordList)
